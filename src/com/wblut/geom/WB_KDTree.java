...
public class WB_KDTree<T extends WB_Point, V> {
	...
	public class WB_KDEntry<T extends WB_Point, V> {
		public T coord;
		public V value;
		public double d2;
	...
	private class WB_KDNode<T extends WB_Point, V> {
		...
		private V lookup(final T point) {
			for (int i = 0; i < _binSize; i++) {
				if (WB_Epsilon.isZeroSq(point.sqDistance(_bin[i].coord))) {
					return _bin[i].value;
				}
			}
			return null;
		...
		private void findNearest(final QueryResult<T, V> heap, final T data) {
			...
			if (_isLeaf) {
				for (int i = 0; i < _binSize; i++) {
					double dist = _bin[i].coord.sqDistance(data);
		...
		private WB_KDEntry<T, V>[] range(final T center, final double r2) {
			...
			int n = 0;
			for (int i = 0; i < _binSize; i++) {
				double d2 = center.sqDistance(_bin[i].coord);
				if (d2 <= r2) {
					_bin[i].d2 = d2;
					tmp[n++] = _bin[i];
				}
	...
	public class QueryResult<T extends WB_Point, V> {
